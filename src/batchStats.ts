...
export type SampleFailureSeedV1 = {
  ...
  seedUsed: number;
  reason: string;
  // parsed out of "(edgeConsidered=.. gateElim=..)" when present
  edgeStats?: Record<string, number>;
};

export type SampleLeverCaseSeedV1 = {
  schemaVersion: 1;
  seed: string;
  seedUsed: number;
  gateDoorId: number;
...
export type BatchPatternSummary = {
  ...
  samples?: {
    failureSeeds?: SampleFailureSeedV1[];
    leverBehindOwnGateSeeds?: SampleLeverCaseSeedV1[];
    leverBlockedByOtherDoorSeeds?: SampleLeverCaseSeedV1[];
  };
};
...
function parseParenKeyVals(reason: string): Record<string, number> | undefined {
  const m = reason.match(/\(([^)]*)\)\s*$/);
  if (!m) return undefined;
  const body = m[1];
  const out: Record<string, number> = {};
  for (const part of body.split(/\s+/g)) {
    const kv = part.split("=");
...
export function aggregateBatchRuns(runs: BatchRunInput[]): BatchSummary {
  ...
  function makeAcc() {
    return {
      ...
      sampleLeverBlockedByOtherDoorSeeds: [] as SampleLeverCaseSeedV1[],
    };
  }
  ...
  const byPattern = new Map<
    string,
    {
      ...
      sampleLeverBlockedByOtherDoorSeeds: SampleLeverCaseSeedV1[];
    }
  >();
  ...
  for (const r of runs) {
    ...
    for (const d of r.patternDiagnostics ?? []) {
      ...
      // --- Sample seed capture (Phase 2.5 drilldowns) -----------------------

      // 1) gateThenOptionalReward failures (want ~4)
      if (name === "gateThenOptionalReward" && !d.ok) {
        if (next.sampleFailureSeeds.length < 4) {
          const reason = String(d.reason ?? "Failed");
          next.sampleFailureSeeds.push({
            schemaVersion: 1,
            seed: r.seed,
            seedUsed: r.seedUsed,
            reason,
            edgeStats: parseParenKeyVals(reason),
          });
        }
      }

      // 2) lever behind own gate (want ~10)
      const leverDiag = (d as any).leverBehindOwnGate as
        | LeverBehindOwnGateDiagV1
        | undefined;

      if (name === "gateThenOptionalReward" && leverDiag) {
        if (
          leverDiag.isBehindOwnGate &&
          next.sampleLeverBehindOwnGateSeeds.length < 10
        ) {
          next.sampleLeverBehindOwnGateSeeds.push({
            schemaVersion: 1,
            seed: r.seed,
            seedUsed: r.seedUsed,
            ...
            reachableIfGateWereOpen: leverDiag.reachableIfGateWereOpen,
            reachableIfAllDoorsWereOpen: leverDiag.reachableIfAllDoorsWereOpen,
            isBehindOwnGate: leverDiag.isBehindOwnGate,
            blockedByOtherDoor: leverDiag.blockedByOtherDoor,
            unreachableEvenIfAllDoorsOpen:
              leverDiag.unreachableEvenIfAllDoorsOpen,
          });
        }

        // 3) blocked by other door (want ~10)
        if (
          leverDiag.blockedByOtherDoor &&
          next.sampleLeverBlockedByOtherDoorSeeds.length < 10
        ) {
          next.sampleLeverBlockedByOtherDoorSeeds.push({
            schemaVersion: 1,
            seed: r.seed,
            seedUsed: r.seedUsed,
            gateDoorId: leverDiag.gateDoorId,
