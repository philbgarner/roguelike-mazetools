...
Non-carving door patterns (Lever→Door, Plate→Door) attempted to find
door sites by scanning for a *single tile adjacent to two different rooms*
(via neighboring regionIds).

With current geometry (rooms labeled, corridors regionId==0),
this condition is often never met — even though corridors clearly connect rooms.

SYMPTOM:
Pattern diagnostics reported:
“Lever pattern: No valid door sites found.”
even with many corridors available.

FIX DIRECTION (APPROVED):

- Door sites are now defined **per corridor**, not per tile.
- Patterns iterate dungeon.meta.corridors.
- Each corridor is mapped to (roomA, roomB) using nearest-room lookup.
- A door tile is chosen *along the corridor path*, not inferred from local adjacency.
- Door placement prefers corridor tiles (regionId==0), but can fall back to floor.
- First and last N tiles of each corridor path are ignored to avoid placing doors
  inside rooms or immediately at room thresholds.
...
- Milestone 2 gates placed with door-site budgeting
...
- Two previously identified ordering bugs are fixed
- Door-site definition bug identified and corrected
- Pattern diagnostics now meaningfully reflect real placement constraints
...
- Apply corridor-based door-site logic to:
  - Lever opens door pattern
  - Plate opens door pattern
  - Door-site budgeting counter
- Tune corridor trimming value (first/last N tiles) if needed
- Verify pattern success rate across multiple seeds

Short-term:

- Add dynamic trimming (“skip until regionId==0”) as an alternative to fixed N
...
- Introduce additional non-carving patterns (hazard gates, timed doors)
- Add player-triggered plates to runtime movement
- Expand circuit logic (thresholds, multi-targets)
